...
export function getBiteMult(s,p){
  // Dragon ON → ล็อกตัวคูณตายตัว
  if(G.dragon&&G.dragon.on){
    if(s===1)return 50;         // HIO
    const d=s-p;
...
export function calcTeamHole(h){
  ...
  // Dragon: ทีมเล่นเพื่อหาแชมป์ ไม่มีเงินไหลระหว่างผู้เล่น
  if(G.dragon&&G.dragon.on)return Array(players.length).fill(0);
